...
function StrategyStep({ mutation, isEnabled, onToggle, config, onConfigChange }) {
  ...
  return (
    <div
      className={`border-b border-[#2a2a2a] px-2 py-1.5 ${hasConfig ? 'cursor-pointer select-none' : ''}`}
    ...
    >
      <div className="flex items-center gap-1.5">
        <input
          type="checkbox"
          checked={isEnabled}
          onChange={e => { e.stopPropagation(); onToggle(mutation.id) }}
          style={{ accentColor: '#00cc66' }}
        />
        <div className="flex-1 min-w-0">
          <div className={`text-xs font-sans ${isEnabled ? 'text-text' : 'text-muted'}`}>
            {mutation.name}
          </div>
        ...
        {hasConfig && (
          <span
            className={`text-[10px] text-muted select-none inline-block shrink-0 ${
              expanded ? 'rotate-90' : ''
            }`}
          >
            &#9654;
      ...
      {expanded && hasConfig && (
        <div className="mt-1.5 pl-6 text-[11px] font-sans">
          {mutation.id === 'synonym_expansion' && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-[10px] text-muted italic">Higher values may increase generation time</span>
            </div>
          )}
          ...
          {mutation.id === 'case_mutation' && (
            <div className="text-muted">No additional config</div>
          )}
        </div>
      )}
    </div>
